...
class _WebkitRendererHelper(QObject):
    ...
    def render(self, url):
        ...
        httpout.write("<!-- Web Rendering Proxy v%s by Antoni Sawicki, modified by RichardG867 -->\n<html>\n<head>\n" % __version__)
        for x in frame.findAllElements('title'):
            httpout.write("<title>%s</title>\n" % x.toPlainText())
        httpout.write("</head>\n<body leftmargin=\"0\" topmargin=\"0\">\n<script>document.write('<span style=\"display: none;\"><img src=\"http://width-' + document.body.clientWidth + '-px.jpg\" width=\"0\" height=\"0\"></span>');</script>\n<img src=\"http://%s\" alt=\"webrender\" usemap=\"#map\" border=\"0\">\n<map name=\"map\">\n" % IMG)
        for x in frame.findAllElements('a'):
            value = x.attribute('href')
            xmin, ymin, xmax, ymax = x.geometry().getCoords() 
            httpout.write("<area shape=\"rect\" coords=\"%i,%i,%i,%i\" alt=\"%s\" href=\"%s\">\n".decode('utf-8', errors='ignore') % (xmin, ymin, xmax, ymax, value, value))
        httpout.write("</map>\n</body>\n</html>\n")
...
class Proxy(SimpleHTTPServer.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        wid_re = re.compile("http://width-[0-9]+-px\.jpg")
